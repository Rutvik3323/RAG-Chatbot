...
from vectorstore import VectorStore # Custom module to store and search text vectors
from chunker import DocumentChunker # Custom module to split documents into chunks
from datetime import datetime
from pathlib import Path

from crewai_tools import QdrantVectorSearchTool # Tool for vector-based search
from transformers import AutoTokenizer, AutoModel # For creating text embeddings
import torch
...
def custom_embeddings(text: str) -> list[float]:
    """
        Purpose: Convert input text into a numerical embedding using a transformer model.
        Design: Mean pooling over token embeddings gives a good balance of performance and quality.
        Assumption: Text is not too long; truncation will handle overly long inputs.
        Limitation: Not ideal for long documents or multi-paragraph inputs.
        """
...
agents_config_path = str(base_path / "src/chatbot/config/agents.yaml")
tasks_config_path = str(base_path / "src/chatbot/config/tasks.yaml")

@CrewBase
class RAGChatbot:
    """Simplified RAG Chatbot with 2 agents and 2 tasks for basic functionality."""
    vector_store = None
    agents_config = agents_config_path  # Add this line to initialize agents_config
    tasks_config = tasks_config_path   # Add this line to initialize tasks_config
    ...
    def answer_question(self, user_query):
        """
               Purpose: Run the RAG pipeline to answer a user query with evaluation.
               Design:
                   - Crew executes retrieval + response
                   - Evaluator rates the quality of the response
               Assumption: Vector store already has data; evaluator works properly.
               Limitation: Basic retries and error handling; no UI feedback or logs.
               """
        try:
            inputs = {
